...
ai_handle_response_hook() {
    ...
    if [[ $c ]]; then
        ...
        if [[ -n "$PENDING_FEEDBACK_ID" ]]; then
            llm feedback-1 --prompt_id "$PENDING_FEEDBACK_ID" "[err:formatting] turn:1" 2>&1>/dev/null
            unset PENDING_FEEDBACK_ID
        elif [[ -z "$HAD_FIRST_RESPONSE" ]]; then
            resp_id=$(sqlite3 "$(llm logs path)" "SELECT id FROM responses ${u:+WHERE prompt LIKE '%U:$u%'} ORDER BY id DESC LIMIT 1")
            llm feedback+1 --prompt_id "$resp_id" "[ok:formatting]" 2>&1>/dev/null
            export HAD_FIRST_RESPONSE=1
        fi

        echo "source ~/ai/ai_tools.sh; $c" > /tmp/ai_code.sh
        script -q -e -c "bash /tmp/ai_code.sh" /tmp/ai_out
        exit_code=$?
        o=$(cat /tmp/ai_out)
        echo -e "\033[35mEXIT CODE:\033[0m $exit_code"
        d=$(sqlite3 "$(llm logs path)" "SELECT conversation_id FROM responses ${u:+WHERE prompt LIKE '%U:$u%'} ORDER BY id DESC LIMIT 1")
    elif [[ -z $HAD_FIRST_RESPONSE ]]; then
        export HAD_FIRST_RESPONSE=1
        o="[SYSTEM WARNING]: No code block detected. Use \`\`\`\` to execute bash or reply 'NO OP'."
        d=$(sqlite3 "$(llm logs path)" "SELECT conversation_id FROM responses ORDER BY id DESC LIMIT 1")
    else
        DONE=1
    fi
}
