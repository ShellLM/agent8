---
title: 'The 300-Byte Autonomous Agent'
description: 'We tested Agent8 against a 3-tool challenge.'
pubDate: 'Feb 14 2026'
...
<MetricsTable items={[
	...
	{ label: 'Code Weight', value: 300, unit: 'bytes' },
	{ label: 'Agent size (Base)', value: 254, unit: 'bytes' },
	{ label: 'Total Tokens', value: 517 },
	{ label: 'Avg Latency', value: 4633, unit: 'ms' },
	{ label: 'Safety Interventions', value: 1 }
]} />
...
```bash
. ~/ai/ai_hooks.sh; [ -t 0 ] || o=$(cat); u=$(uuidgen); \
while :; do \
  eval $P; \
  r=$(llm -s "$(<"$0")" "$@" <<< "U:$u $o"); \
  c=$(sed -n '/^````/,/^````/{//!p}' <<< "$r"); \
