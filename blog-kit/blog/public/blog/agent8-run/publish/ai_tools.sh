...
spawn() {
	...
	# Tofix: I don't know how this should work but it definitaly should NOT be using llm-last-cid for the conversation ID, as that will cause all sub-agents to use whatever is the most recent conversation, which could be completely unrelated to the task at hand. 
	# We need to either drop the --cid causing a new conversation ID to be generated for each sub-agent or pass the parent conversation ID explicitly if we want them to be part of the same conversation thread.
